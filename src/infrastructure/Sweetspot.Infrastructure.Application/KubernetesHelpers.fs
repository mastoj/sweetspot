...
let getStackRef stackName =
    if Map.containsKey stackName mutableStackMap |> not then
        mutableStackMap <-
            mutableStackMap
            |> Map.add stackName (StackReference(stackName))
...
let getAcrRegistryName stack =
    let fullName =
        stack |> getStackOutput "registryLoginServer"
...
let getK8sProvider providerName namespaceName clusterConfig =
    if k8sProvider.IsNone then
        k8sProvider <-
            Some(Provider(providerName, ProviderArgs(KubeConfig = io clusterConfig, Namespace = input namespaceName)))
    k8sProvider.Value

let getSha () =
    let repoPath =
        Repository.Discover(System.Environment.CurrentDirectory)
...
let createDeployment (stack: StackReference) (k8sProvider: Provider) (deploymentConfig: DeploymentConfig) =
    ...
    let fullImageName =
        acrRegistry.Apply(fun acr -> sprintf "%s/%s:%s" acr imageName sha)

    let (DeploymentName deploymentName) = deploymentConfig.Name
    let (Replicas replicas) = deploymentConfig.Replicas

    let options =
        deploymentConfig.SetCustomResourceOptions(CustomResourceOptions(Provider = k8sProvider))
                                                       ...
                                                       inputList [ input
                                                                       (ContainerArgs
                                                                           (Name = input deploymentName,
                                                                            ...
                                                                            Env =
                                                                                inputList deploymentConfig.EnvVariables,
                                                                            Ports =
                                                                                inputList [ input
                                                                                                (ContainerPortArgs
                                                                                                    (ContainerPortValue =
...
let createService (k8sProvider: Provider) (serviceConfig: ServiceConfig) =
    ...
    let options =
        serviceConfig.SetCustomResourceOptions(CustomResourceOptions(Provider = k8sProvider))

    let serviceArgs =
        ServiceArgs(Metadata = input (ObjectMetaArgs(Name = input serviceName)), Spec = input (ServiceSpecArgs()))
...
let createApplicationConfig (ApplicationName applicationName) imageName =
    ...
    let deploymentConfig =
        { DeploymentConfig.InitConfig (DeploymentName applicationName) (imageName) (Replicas 1) with
              Labels = (InputMap.List deploymentLabels) }

    let serviceConfig =
        { ServiceConfig.InitConfig(ServiceName applicationName) with
              Selector = AppSelector(InputMap.List deploymentLabels) }
...
let createApplication (stack: StackReference) (k8sProvider: Provider) (applicationConfig: ApplicationConfig) =
    let deployment =
        applicationConfig.DeploymentConfig
        |> createDeployment stack k8sProvider

    let service =
        applicationConfig.ServiceConfig
        |> createService k8sProvider
...
let createSecret (k8sProvider: Provider) name (inputMap: InputMap) =
    let options =
        CustomResourceOptions(Provider = k8sProvider)

    Secret(name, SecretArgs(Data = inputMap.ToInputMap()), options = options)
    ...
    { appConfig with
          DeploymentConfig =
              { appConfig.DeploymentConfig with
                    EnvVariables =
                        envVariable
                        :: appConfig.DeploymentConfig.EnvVariables } }
                             ...
                             (SecretKeyRef =
                                 input
                                     (SecretKeySelectorArgs
                                         (Name = io (secret.Metadata.Apply(fun m -> m.Name)), Key = input key))))))
