...
let getImageRegistry stack =
    ...
    let adminUserName =
        stack |> getStackOutput "registryAdminUsername"

    let adminPassword =
        stack |> getStackOutput "registryAdminPassword"

    ImageRegistry(Password = io adminPassword, Server = io loginServer, Username = io adminUserName)
...
let publishImages stack (paths: string list) =
    ...
    |> List.map (fun path ->
        ...
        let fullImageName =
            imageRegistry.Server.Apply(fun acr -> sprintf "%s/%s:%s" acr imageName sha)

        Image
            (imageName,
             ImageArgs(Build = inputUnion1Of2 path, ImageName = io fullImageName, Registry = input imageRegistry)))
